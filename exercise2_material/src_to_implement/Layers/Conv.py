...
class Conv(BaseLayer):
    ...
    def backward(self, error_tensor):
        ...
        if np.ndim(self.weights) == 4:  # 2D case
            ...
            for i in range(self.input_tensor.shape[0]):  # batch size
                for j in range(self.error_tensor.shape[1]):  # error tensor channel no / no of filters in forward
                    for k in range(self.input_tensor.shape[1]):  # input tensor channel no / no of filters in backward path
